(function () {
  ...
  const copy = {
    en: {
      ...
      whatsappOnly: "WhatsApp opened. Backend saving needs the production API.",
      serverMissing: "Booking server is not active yet. Send the WhatsApp request to book now.",
  ...
  function bookingMessage(reference, quote) {
    ...
    const priceLine = quote.quoteOnly ? "Exact quote requested" : `${money(quote.total)} TOTAL / per vehicle`;
    return [
